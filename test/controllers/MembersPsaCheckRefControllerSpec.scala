...
class MembersPsaCheckRefControllerSpec extends SpecBase {
  ...
  "Member Psa Check Ref Controller" - {
    "must return OK and the correct view for a GET" in {
      val application = applicationBuilder(userAnswers = emptyUserAnswers).build()

      running(application) {
        val request = FakeRequest(GET, routes.MembersPsaCheckRefController.onPageLoad().url)

        val result = route(application, request).value

        val view = application.injector.instanceOf[MembersPsaCheckRefView]
        val backLinkRoute = routes.MembersNinoController.onPageLoad(NormalMode).url
