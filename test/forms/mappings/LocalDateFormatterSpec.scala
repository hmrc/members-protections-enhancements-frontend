    ...
    "return form errors" - {
      ...
      "the day is invalid" in {
        formatter.validateDayMonthYear("date", Some("f"), Some("11"), Some("2000")) shouldBe
          Seq(FormError("date.day", dayInvalidKey, Seq("day")))
      }

      "the month is invalid" in {
        formatter.validateDayMonthYear("date", Some("11"), Some("19"), Some("2000")) shouldBe
          Seq(FormError("date.month", monthInvalidKey, Seq("month")))
      }

      "the month text is invalid" in {
        formatter.validateDayMonthYear("date", Some("11"), Some("f"), Some("2000"), true) shouldBe
          Seq(FormError("date.month", monthTextInvalidKey, Seq("month")))
      }

      "the year is invalid" in {
        formatter.validateDayMonthYear("date", Some("11"), Some("11"), Some("f")) shouldBe
          Seq(FormError("date.year", yearInvalidKey, Seq("year")))
      }

      "the month and year are invalid" in {
        formatter.validateDayMonthYear("date", Some("11"), Some("20"), Some("0")) shouldBe Seq(
          FormError("date.month", monthInvalidKey, Seq("month", "year")),
          FormError("date.year", yearInvalidKey, Seq("month", "year"))
        )
      }
      ...
      "the day and month are invalid" in {
        formatter.validateDayMonthYear("date", Some("5.6"), Some("9.9"), Some("2000")) shouldBe Seq(
          FormError("date.day", dayInvalidKey, Seq("day", "month")),
          FormError("date.month", monthInvalidKey, Seq("day", "month"))
        )
      }
