...
class DataRetrievalActionImpl @Inject() (
  ...
  override protected def transform[A](request: IdentifierRequest[A]): Future[DataRequest[A]] =
    sessionRepository.get(request.userDetails.userId).map {
      case Some(value) => DataRequest(request.request, request.userDetails, value)
      case None => DataRequest(request.request, request.userDetails, UserAnswers(request.userDetails.userId))
    }
}
